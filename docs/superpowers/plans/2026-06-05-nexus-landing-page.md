# Nexus AI Landing Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `/landing` public route for Nexus AI with 9 sections following the Prymeira inline-styles dark theme, including a detailed Platforms section (Meta Ads + Google Ads) for Google Ads account review approval.

**Architecture:** Single new file `components/NexusLandingPage.tsx` with inline styles, no Tailwind, matching the FlowcutLandingPage pattern. App.tsx gets a one-line route check. A symlink in the Hub's `apps/` directory registers the project in the monorepo.

**Tech Stack:** React 19, TypeScript, lucide-react (already installed), inline styles only.
...
| Action | Path | Purpose |
|--------|------|---------|
| Create | `components/NexusLandingPage.tsx` | All 9 landing page sections |
| Modify | `App.tsx` lines 911-918 | Add `/landing` route check |
| Create | `Hub — Prymeira Account/apps/nexus-ai` (symlink) | Register app in Hub monorepo |

---

### Task 1: Create symlink in Hub

**Files:**
- Create: `Hub — Prymeira Account/apps/nexus-ai` → symlink to `../../../../nexus-ai-main`

- [ ] **Step 1: Create the symlink**

```bash
ln -s "$(pwd)" "/Users/yohannreimer/Downloads/Locais/Prymeira/Hub — Prymeira Account/apps/nexus-ai"
```

Run from inside `/Users/yohannreimer/Downloads/nexus-ai-main`.

- [ ] **Step 2: Verify symlink**

```bash
ls -la "/Users/yohannreimer/Downloads/Locais/Prymeira/Hub — Prymeira Account/apps/"
```

Expected output includes:
```
nexus-ai -> /Users/yohannreimer/Downloads/nexus-ai-main
```

---

### Task 2: Create `NexusLandingPage.tsx`

**Files:**
- Create: `components/NexusLandingPage.tsx`

This is the entire file, broken into sub-steps per section group.

- [ ] **Step 1: Write file header, tokens, types, and `useIsMobile` hook**

Create `components/NexusLandingPage.tsx` with:
...
- [ ] **Step 4: Write `Problema` component**
...
- [ ] **Step 5: Write `Plataformas` component**

This is the most important section for Google Ads review. Append to the file:
...
- [ ] **Step 6: Write `Funcionalidades` component**
...
- [ ] **Step 7: Write `ComoFunciona` component**
...
- [ ] **Step 9: Write `CtaFinal`, `Footer`, and export `NexusLandingPage`**
...
```bash
cd /Users/yohannreimer/Downloads/nexus-ai-main && npx tsc --noEmit 2>&1 | head -30
```

Expected: no errors related to `NexusLandingPage.tsx`. Fix any type errors before proceeding.

---

### Task 3: Wire `/landing` route in `App.tsx`

**Files:**
- Modify: `App.tsx` lines 911-918

- [ ] **Step 1: Add import for NexusLandingPage**

In `App.tsx`, after the existing imports (around line 15), add:

```tsx
import { NexusLandingPage } from './components/NexusLandingPage';
```

- [ ] **Step 2: Add route check**

In `App.tsx`, find the `App` component (around line 911):
...
```bash
cd /Users/yohannreimer/Downloads/nexus-ai-main && npx tsc --noEmit 2>&1 | head -20
```

Expected: no errors.

- [ ] **Step 4: Start dev server and verify `/landing` renders**

```bash
cd /Users/yohannreimer/Downloads/nexus-ai-main && npm run dev
```

Open `http://localhost:5173/landing` in browser. Expected: landing page with Navbar, Hero (gold headline), 9 sections visible, no console errors.
...
**Spec coverage:**
- ✅ Symlink Hub/apps/nexus-ai → nexus-ai-main (Task 1)
- ✅ NexusLandingPage.tsx with all 9 sections (Task 2)
- ✅ /landing route in App.tsx (Task 3)
- ✅ Meta Ads platform card with OAuth, metrics, usage, security badge (Task 2 Step 5)
- ✅ Google Ads platform card with OAuth, metrics, usage, security badge (Task 2 Step 5)
- ✅ useIsMobile + isMobile prop on all sections (Task 2 Steps 1-9)
- ✅ Inline styles only, no Tailwind (all steps)
- ✅ Design tokens BG/SURFACE/BORDER/ACCENT/TEXT/TEXT_SOFT match spec (Task 2 Step 1)
- ✅ Planos toggle mensal/anual (Task 2 Step 8)
- ✅ No new dependencies (lucide-react not needed — replaced with inline SVGs)

**Type consistency:**
- `NexusLandingPage` exported as named export — matches import in Task 3 Step 1 ✅
- All section components receive `{ isMobile: boolean }` — consistent across steps ✅
- `PLANS` array typed implicitly, `billing` state typed as `'monthly' | 'annual'` ✅
- `PLATFORMS` array uses inline JSX for `icon` field — consistent in Step 5 ✅
